...
export default function FlipTracker({ flips, setFlips }) {
  ...
  const saveFlip = () => {
    if (!form.title || !form.paid || !form.sold) return
    if (editId !== null) {
      setFlips(prev => prev.map(f => f.id === editId ? { ...form, id: editId } : f))
      setEditId(null)
    } else {
      setFlips(prev => [...prev, { ...form, id: Date.now() }])
    }
    setForm(EMPTY_FORM)
    setShowForm(false)
  }
      ...
      {showForm && (
        <div className="flip-form-card">
          ...
          <div className="flip-form-grid">
            ...
            <div className="form-field">
              <label>Main CPU</label>
              <input className="input-mono" placeholder="e.g. Ryzen 5 5600X" value={form.cpu} onChange={e => updateForm('cpu', e.target.value)} />
            </div>
            <div className="form-field">
              <label>Main GPU</label>
              <input className="input-mono" placeholder="e.g. RTX 3070" value={form.gpu} onChange={e => updateForm('gpu', e.target.value)} />
            </div>
            <div className="form-field">
              <label>What I Paid</label>
          ...
          {form.paid && form.sold && (
            <div className="form-profit-preview">
              {(() => {
                const p = parseFloat(form.sold) - parseFloat(form.paid)
                const pct = parseFloat(form.paid) > 0 ? (p / parseFloat(form.paid)) * 100 : 0
                return (
          ...
          <div className="form-actions">
            ...
            <button className="btn btn-primary" onClick={saveFlip} disabled={!form.title || !form.paid || !form.sold}>
